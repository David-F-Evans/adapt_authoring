...
var inputHelpers = {
  ...
  booleanValidator: function(v) {
    console.log('booleanValidator:', v);
    return true;
  },
  toBoolean: function(v) {
    // accept yes/y, no/n, true/t, false/f
    console.log('toBoolean:', v);
    if(/(Y|y)[es]*/.test(v)) return true;
    if(/(T|t)[rue]*/.test(v)) return true;
    if(/(N|n)[o]*/.test(v)) return false;
    if(/(F|f)[alse]*/.test(v)) return false;
    return false;
  },
  passwordBefore: function(v) {
    // HACK prompt adds a blank line when hidden/replace attrs are set
